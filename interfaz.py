...
from aesC import cifrarAes,descifrarAes
class Interfaz:
    ...
    def cifrarM(self):
        ...
        if(len(cadena.strip())<1):
            ...
            res,nonce,tag = cifrarAes(cifrar64(cadena).encode(),key)
            self.resC = res
            #res,nonce,tag = cifrarAes(b'https://www.saes.upiicsa.ipn.mx/',key)
            self.nonce = nonce
    ...
    def descifrarM(self):
        ...
        if(len(cadena.strip())<1):
            ...
            key = b'Sixteen byte key'
            resA = descifrarAes(self.resC,self.nonce,key)
            resB = "".join( chr(x) for x in resA)
            res = descifrar64(resB)
